...
import (
	...
	// This Service
	"github.com/adamryman/ambition-truss/service/DONOTEDIT"
...
func main() {
	...
	*/
	...
	var createactionEndpoint endpoint.Endpoint
	{
		...
		//createactionLogger := log.NewContext(logger).With("method", "CreateAction)")

		createactionEndpoint = addsvc.MakeCreateActionEndpoint(service)
		//createactionEndpoint = opentracing.TraceServer(tracer, "CreateAction)")(createactionEndpoint)
		//createactionEndpoint = addsvc.EndpointInstrumentingMiddleware(createactionDuration)(createactionEndpoint)
		//createactionEndpoint = addsvc.EndpointLoggingMiddleware(createactionLogger)(createactionEndpoint)
	}

	var createoccurrenceEndpoint endpoint.Endpoint
	{
		//createoccurrenceDuration := duration.With(metrics.Field{Key: "method", Value: "CreateOccurrence})"
		//createoccurrenceLogger := log.NewContext(logger).With("method", "CreateOccurrence)")

		createoccurrenceEndpoint = addsvc.MakeCreateOccurrenceEndpoint(service)
		//createoccurrenceEndpoint = opentracing.TraceServer(tracer, "CreateOccurrence)")(createoccurrenceEndpoint)
		//createoccurrenceEndpoint = addsvc.EndpointInstrumentingMiddleware(createoccurrenceDuration)(createoccurrenceEndpoint)
		//createoccurrenceEndpoint = addsvc.EndpointLoggingMiddleware(createoccurrenceLogger)(createoccurrenceEndpoint)
	}
